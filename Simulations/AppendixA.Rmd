---
title: "Appendix A: Simulation of winding survey bias"
output: 
  pdf_document:
    fig_caption: false

---
Straight line survey routes were infeasible due to impassible terrain and off-road restrictions; thus surveys were designed to maximize coverage of the Talek region and Mara Triangle.
...
The results of the simulation study show that the winding survey is slightly more biased (8.87 % more biased) for the number of groups within the sampling boundary when compared to the straight survey. This also holds true for the number of groups within the enitre study region (winding surveys are 6.44% more biased). However, abundance within the sampling boundary and for the entire study region are similar between the 2 survey methods, and the bias for the sacle parameter, $\sigma$, was less bias than the straight survey (9.04% less biased). We concluded that the winding survey design did not increase bias enough to be an issue. Additionally, biases affected estimates similarly between management regimes.

Below is the annotated code for the simulation study for a single iteration and sub iteration. To see complete code for simulation study, please go to [GitHub](https://github.com/farrmt/HMSDS/tree/master/Simulations) or Zenodo.
...
Run model
```{r, message=FALSE}
windingM <- jags(data = windingD, inits = inits, parameters.to.save = params, model.file = "ssds.txt", 
...
colnames(bias) <- c("Winding True", "Winding Est", "Winding Bias", "Straight True", "Straight Est", "Straight Bias" )
rownames(bias) <- c("Groups Within", "Abundance Within", "Groups", "Abundance", "Sigma")
```
...
**Table 4.** Estimated and true values with associated relative bias for winding and straight surveys for a single iteration and sub-iteration.
```{r, comment=NA, echo=FALSE}
bias
```
