...
We generated 100 datasets for simulating the locations of groups and group sizes and ran 10 sub-iterations to sub-sample for each sampling design for a total of 1000 simulations for each sampling design. For each iteration, observations (groups not individuals) are distributed uniformly across the sampling area. Group sizes are then simulated for each group. For each sub-iteration, the iteration of uniformly distributed observations is resampled. 

The results of the simulation study show that the winding survey is slightly more biased (8.87 % more biased) for the number of groups within the sampling boundary when compared to the straight survey (Table 1). This also holds true for the number of groups within the enitre study region (winding surveys are 6.44% more biased). However, abundance within the sampling boundary and for the entire study region are similar between the two survey methods, and there is no increase in bias for the scale parameter, $\sigma$, which influences detection. We concluded that the winding survey design did not have a signifcant enough increase in bias to impact our results. Additionally, we assume that any potential biases caused by winding transects would have a similar influence in both management regions and would only affect absolute, but not relative, abundance estimates.
...
Hiby, L. & Krishna, M.B. (2001) Line transect sampling from a curving path. Biometrics, 57, 727â731.